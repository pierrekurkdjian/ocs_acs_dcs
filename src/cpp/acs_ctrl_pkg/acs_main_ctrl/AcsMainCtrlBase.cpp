...
#include "AcsMainCtrlSetup.h"
#include "AcsMainCtrlBase.h"

using namespace std;
using namespace gmt;

AcsMainCtrlBase::AcsMainCtrlBase(
...
void AcsMainCtrlBase::create_state()
{
    ...
    new_input_dataio(analog_out_ext,            "analog_out_ext",        analog_out_t());
    new_input_dataio(power_supply_ext,          "power_supply_ext",      std::array<power_supply_t, 4>());
    new_input_dataio(resistor_bridge_status_ext, "resistor_bridge_status_ext", std::array<resistor_bridge_status_t, 7>());

    //create the output dataio of the class, and assign the default value
    new_output_dataio(digital_out,               "digital_out",           digital_out_t());
    new_output_dataio(analog_out,                "analog_out",            analog_out_t());
    new_output_dataio(resistor_bridge_control,   "resistor_bridge_control", std::array<resistor_bridge_control_t, 7>());
    new_output_dataio(digital_in_ext,            "digital_in_ext",        digital_in_t());
    new_output_dataio(analog_in_ext,             "analog_in_ext",         analog_in_t());
    new_output_dataio(resistor_bridge_control_ext, "resistor_bridge_control_ext", std::array<resistor_bridge_control_t, 7>());
...
void AcsMainCtrlBase::configure_from_file (const std::string& fname)
{
    try
    {
...
void AcsMainCtrlBase::setup_state(ComponentSetup& config)
{
